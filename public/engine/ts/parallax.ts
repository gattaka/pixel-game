namespace Lich {
    export class Parallax extends PIXI.Container {
        ...
        private shift() {
            ...
            // Dirt back
            self.dirtBackSprite.x = Utils.floor(((self.offsetX * Parallax.DIRT_MULT) % self.dirtBackSprite.originalWidth) - self.dirtBackSprite.originalWidth);
            let tillRepeatPointY = Utils.floor((self.offsetY + Parallax.DIRT_START) * Parallax.DIRT_MULT);
            if (tillRepeatPointY < 0) {
                self.dirtBackSprite.y = Utils.floor(tillRepeatPointY % self.dirtBackSprite.originalHeight) - self.dirtBackSprite.originalHeight;
            } else {
