...
(function (Lich) {
    var AbstractEnemy = (function (_super) {
        ...
        AbstractEnemy.prototype.isPlayerInReach = function (world) {
            ...
            var ex2 = this.x + this.fixedWidth - this.collXOffset;
            var ey1 = this.y + this.collYOffset;
            var ey2 = this.y + this.fixedHeight - this.collYOffset;
            // player
            var hero = world.hero;
            var px1 = hero.x + hero.collXOffset;
            var px2 = hero.x + hero.fixedWidth - hero.collXOffset;
            var py1 = hero.y + hero.collYOffset;
            var py2 = hero.y + hero.fixedHeight - hero.collYOffset;
            ...
            if ((ex1 >= px1 && ex1 <= px2 || ex2 >= px1 && ex2 <= px2 || px1 >= ex1 && px1 <= ex2 || px2 >= ex1 && px2 <= ex2)
                ...
                var heroFeet = world.hero.y + world.hero.fixedHeight - world.hero.collYOffset;
                var enemyHead = this.y;
                var enemyFeet = this.y + this.fixedHeight;
        ...
        AbstractEnemy.prototype.dropLoot = function (world, invKey, quant, batch) {
            ...
            for (var i = 0; i < quant; i++) {
                ...
                var xjitter = Math.random() * (this.fixedWidth - Lich.Resources.PARTS_SIZE);
                var yjitter = Math.random() * (this.fixedHeight - Lich.Resources.PARTS_SIZE);
            ...
            if (this.getCurrentHealth() > 0) {
                ...
                if (this.isPlayerInReach(world)) {
                    ...
                    var verticalStrategy = function (nextX) {
                        if ((world.hero.y + world.hero.fixedHeight) > (_this.y + _this.fixedHeight)) {
                            // pokud je hráč níž než já (vzdálenost je obrácená)
                            var col = world.isCollision(nextX, _this.y + _this.fixedHeight - Lich.Resources.TILE_SIZE);
                            ...
                            if (nextX != 0 && (world.isCollision(nextX, _this.y + _this.fixedHeight + Lich.Resources.TILE_SIZE).hit == false
                                || world.isCollision(nextX, _this.y + _this.fixedHeight - Lich.Resources.TILE_SIZE).hit)) {
